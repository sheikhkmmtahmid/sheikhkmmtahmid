...
|---|---|---|
| 🤖 [Explainable AI Hiring Intelligence Platform](https://github.com/sheikhkmmtahmid/Explainable-AI-Hiring-Intelligence-Platform) | Production HR-tech platform with 12-app modular Django REST API generating 384-dim SBERT embeddings to cosine-rank 3,000+ candidates per job posting. Fuses SHAP and LIME (60/40) into a unified explainability report per match, computes disparate impact ratio and 4/5 rule across gender, ethnicity, and age groups, and orchestrates async Celery pipeline for CV parsing, embedding generation, and batch matching | `Python` `Django` `DRF` `SBERT` `SHAP` `LIME` `Celery` `Redis` `PostgreSQL` `React` |
